...
function dissForge() {
    ...
    function canonizeName(name) {
        return name.split(/[^\w]/).map(function(s,idx) {
            if (idx===0) return s;
    ...
    function requireWrapper(name, host) {
        //
        // Shorthand to diss.register.module('name',require('name'));
        //     
        module(name, ( host || require.main ).require(name));
        return diss;
    }
    ...
    function loadDependencies(pkg ,main) {
        ...
        Object.keys(pkg.dependencies || {}).forEach( name => requireWrapper(name, from) );
        return diss;
    }
